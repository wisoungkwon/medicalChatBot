...
- [x] ~~임베딩 가중치~~ — **청킹으로 해결.** 실제 한계는 512가 아니라 **128토큰**이었고,
      100건 전부가 이를 넘겨 각 문서의 앞 4%만 벡터가 되고 있었습니다(중앙값 5,700토큰).
      병명·원인·치료는 어느 문서에서도 임베딩된 적이 없었고, 증상 3회 반복 가중치는
      첫 128토큰이 동일하므로 아무 효과가 없었습니다. 질병 1건 = 벡터 1개 →
      **조각 4,642개**로 바꾼 뒤 `Recall@1 52.2% → 65.4%`, 되묻기 `63.7% → 48.1%`
...
인덱싱 방식이나 임베딩 모델을 바꾸면 **숫자로 확인하고** 임계값을 다시 잡으세요.

```bash
python Flask_API/eval/build_eval_set.py          # 평가셋 생성 (1회)
python Flask_API/eval/eval_retrieval.py          # 현재 성능
python Flask_API/eval/eval_retrieval.py --tune   # + 임계값 후보별 표
```

평가셋은 질병 JSON 의 `증상.supplement` 발화 **1,920건**입니다. 질병당 10~20개의
사용자 말투 문장이고 질병 간 중복이 0건이라 정답이 모호하지 않습니다.
LLM 은 호출하지 않고 검색만 잽니다.

**2026-09-18 청킹 전후** (같은 평가셋, 질병 100종)
...
발화가 문서 뒤쪽에 있을수록 검색이 안 되던 문제가 핵심이었습니다. 이 격차가
잘림의 크기를 그대로 보여줍니다:

| supplement 내 위치 | 청킹 전 Recall@1 | 청킹 후 |
|---|---:|---:|
| 0–4 (문서 앞) | 72.4% | 71.4% |
| 10+ (문서 뒤) | 43.6% | **63.4%** |

> ⚠️ 질의로 쓰는 발화가 색인 대상 문서 안에도 들어 있어(누설) 절대값은 낙관적입니다.
> **같은 평가셋에서의 전후 비교**와 **위치 구간별 격차**를 보는 용도입니다.

### 되묻기 임계값을 왜 0.82 로 두었나

`HIGH_CONF_THRESHOLD` 미만이면 되묻고, 추가 증상이 붙으면 **확신도와 무관하게**
답합니다. 즉 되묻기는 한 번뿐이고, 그 한 번이 값어치를 하는지가 관건입니다.

되묻기는 검색을 크게 개선합니다 — 같은 질병의 다른 발화를 추가 증상으로 덧붙여
측정한 결과:
...
임계값별 **최종** 결과 (바로 답한 사람 + 되물은 뒤 답한 사람 전체):

| HIGH | 되묻기 | 바로답한쪽 | 되물은쪽 | 최종 1위 | 최종 후보 |
|---:|---:|---:|---:|---:|---:|
| 0.74 | 48.9% | 93.6% | 95.0% | 82.4% | 94.3% |
| 0.80 | 69.7% | 96.4% | 95.9% | 84.7% | 96.0% |
| **0.82** | **79.0%** | **97.0%** | **96.2%** | **85.6%** | **96.4%** |
| 0.85 | 90.8% | 99.4% | 96.6% | 87.0% | 96.8% |

**0.80 아래에서는 되물은 쪽 성적이 바로 답한 쪽보다 높습니다** — 답하지 말았어야 할
질의에 답하고 있었다는 뜻입니다. 두 값이 교차하는 지점이 0.80 근처입니다.
0.85 이상은 사실상 "항상 되묻기"인데 최종 정확도는 0.4%p 더 얻는 데 그칩니다.

> LLM 은 top-1 이 아니라 **top-5 후보를 통째로** 받아 그 안에서 고릅니다
> (`MAX_DISEASES=5`). 그래서 실질 상한은 Recall@1 이 아니라 "후보 적중" 입니다.
